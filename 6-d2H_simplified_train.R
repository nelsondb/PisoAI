...
d2H.xgb.vl.model.inputs<-colnames(dplyr::select(x,-c("d2Hf")))
write.csv(d2H.xgb.vl.model.inputs,
          file="d2H/d2H.xgb.vl.model.inputs.csv")
...
d2H.xgb28vl.pp <- caret::preProcess(subset(training,select=-c(d2Hf)), # last column is the outcome 
                                  method = c("center", "scale", "nzv"))
saveRDS(d2H.xgb28vl.pp,file="d2H/d2H.xgb28vl.pp")

training.tr <- predict(d2H.xgb28vl.pp, newdata = subset(training,select=-c(d2Hf)))
training.target<-subset(training,select=c(d2Hf))

test.tr <- predict(d2H.xgb28vl.pp, newdata = subset(test,select=-c(d2Hf)))
test.target<-subset(test,select=c(d2Hf))

watch.tr <- predict(d2H.xgb28vl.pp, newdata = subset(watch,select=-c(d2Hf)))
...
grid.number<-1
start.time <- Sys.time()
xgbGrid <- expand.grid(max_depth = c(7,9,11),
                       eta = c(0.1),
                       rate_drop = c(0.01,0.02,0.03,0.1,0.2),
                       skip_drop = c(0.01,0.02,0.03,0.1,0.2),
...
write.csv(xgb_grid28vl, file = "d2H/d2H.xgb_grid28vl.csv")
...
training.plot.d2H.xgb28vl<-
  ...
  theme_bw()
training.plot.d2H.xgb28vl
pdf("d2H/training.plot.d2H.xgb28vl.pdf",width=8,height=8)
training.plot.d2H.xgb28vl
dev.off()

saveRDS(bst,file="d2H/d2H.xgb28vl.model")
...
write.csv(importance, file = paste("d2H/xgb.importance",tuneround,".csv"))

write.csv(xgbDart.tuning28vl, file = "d2H/d2H.xgbDart.tuning28vl.csv")  



set.seed(1234)
iA<-test
iA$xgb28vl.d2H.m<-predict(bst, dtest, nrounds = nrounds)
set.seed(1234)
iB<-watch
iB$xgb28vl.d2H.m<-predict(bst, dwatch, nrounds = nrounds)
set.seed(1234)
iC<-training
iC$xgb28vl.d2H.m<-predict(bst, dtrain, nrounds = nrounds)
...
write.csv(GNIP.xgb28vl, file="d2H/GNIP.d2H.xgb28vl.csv")
