...
x<-GNIP.climate.obs[,c("Elevation","Longitude","Latitude","lat2","Site",#direct GNIP station data or derived products that can be replicated in gridded data sets like CRU, EOBS, DEM
                       ...
                       "d18Of" )] #response variable

d18O.xgb.model.inputs<-colnames(dplyr::select(x,-c("d18Of")))
write.csv(d18O.xgb.model.inputs,
          file="d18O/d18O.xgb.model.inputs.csv")
...
d18O.xgb28.pp <- caret::preProcess(subset(training,select=-c(d18Of)), # last column is the outcome 
                            method = c("center", "scale", "nzv"))

saveRDS(d18O.xgb28.pp,file="d18O/d18O.xgb28.pp")

training.tr <- predict(d18O.xgb28.pp, newdata = subset(training,select=-c(d18Of)))
training.target<-subset(training,select=c(d18Of))

test.tr <- predict(d18O.xgb28.pp, newdata = subset(test,select=-c(d18Of)))
test.target<-subset(test,select=c(d18Of))

watch.tr <- predict(d18O.xgb28.pp, newdata = subset(watch,select=-c(d18Of)))
...
write.csv(xgb_grid28, file = "d18O/d18O.xgb_grid28.csv")
...
training.plot.d18O.xgb28<-
  ...
  theme_bw()
training.plot.d18O.xgb28
pdf("d18O/training.plot.d18O.xgb28.pdf",width=8,height=8)
training.plot.d18O.xgb28
dev.off()

saveRDS(bst,file="d18O/d18O.xgb28.model")
...
write.csv(importance, file = paste("d18O/xgb.importance",tuneround,"-full-loctrain.csv"))

write.csv(xgbDart.tuning28, file = "d18O/d18O.xgbDart.tuning28.csv")  


set.seed(1234)
iA<-test
iA$xgb28.d18O.m<-predict(bst, dtest, nrounds = nrounds)
set.seed(1234)
iB<-watch
iB$xgb28.d18O.m<-predict(bst, dwatch, nrounds = nrounds)
set.seed(1234)
iC<-training
iC$xgb28.d18O.m<-predict(bst, dtrain, nrounds = nrounds)
...
write.csv(GNIP.xgb28, file="d18O/GNIP.d18O.xgb28.csv")
