...
d2H.pp <- caret::preProcess(subset(training,select=-c(d2Hf)), # last column is the outcome 
                            method = c("center", "scale"))
d2H.pp #this will show which columns were centered and scaled
saveRDS(d2H.pp,file="d2H/d2H.pp")


training.tr <- predict(d2H.pp, newdata = subset(training,select=-c(d2Hf)))
training.target<-subset(training,select=c(d2Hf))


test.tr <- predict(d2H.pp, newdata = subset(test,select=-c(d2Hf)))
test.target<-subset(test,select=c(d2Hf))

watch.tr <- predict(d2H.pp, newdata = subset(watch,select=-c(d2Hf)))
...
write.csv(xgb_grid30, file = "d2H/d2H.xgb_grid30.csv")
...
saveRDS(bst,file="d2H/d2H.model1RDS")
d2H.featureIDs<-as.character(bst$feature_names)
saveRDS(d2H.featureIDs,file="d2H/d2H.featureIDs")
xgb.save(bst,"d2H/d2H.model1XGB")
...
write.csv(xgbDart.tuning30, file = "d2H/d2H.xgbDart.tuning30.csv")  
...
set.seed(1234)
iA<-test.import
iA$d2H.m<-predict(bst, dtest, nrounds = nrounds)
set.seed(1234)
iB<-watch.import
iB$d2H.m<-predict(bst, dwatch, nrounds = nrounds)
set.seed(1234)
iC<-training.import
iC$d2H.m<-predict(bst, dtrain, nrounds = nrounds)
...
write.csv(GNIP, file="d2H/GNIP.d2H.csv")
