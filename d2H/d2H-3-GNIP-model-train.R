...
d2H.pp.3 <- caret::preProcess(subset(training,select=-c(d2Hf)), # last column is the outcome 
                            method = c("center", "scale"))
d2H.pp.3 #this will show which columns were centered, scaled, YJ'd, and ignored
saveRDS(d2H.pp.3,file="d2H/d2H.pp.3")


training.tr <- predict(d2H.pp.3, newdata = subset(training,select=-c(d2Hf)))
training.target<-subset(training,select=c(d2Hf))


test.tr <- predict(d2H.pp.3, newdata = subset(test,select=-c(d2Hf)))
test.target<-subset(test,select=c(d2Hf))

watch.tr <- predict(d2H.pp.3, newdata = subset(watch,select=-c(d2Hf)))
...
write.csv(xgb_grid30, file = "d2H/d2H.xgb_grid30.3_srv.csv",row.names=F)
# xgb_grid30<-read.csv(file = "d2H/d2H.xgb_grid30.3_srv.csv")
...
saveRDS(bst,file="d2H/d2H.model3RDS")
d2H.featureIDs<-as.character(bst$feature_names)
saveRDS(d2H.featureIDs,file="d2H/d2H.featureIDs.3")
xgb.save(bst,"d2H/d2H.model3XGB")
...
write.csv(importance, file = paste("d2H/xgb.importance.3",tuneround,".csv"))

write.csv(xgbDart.tuning30, file = "d2H/d2H.xgbDart.tuning30.3.csv")  
...
set.seed(1234)
iA<-test.import
iA$d2H.m3<-predict(bst, dtest, nrounds = nrounds)
set.seed(1234)
iB<-watch.import
iB$d2H.m3<-predict(bst, dwatch, nrounds = nrounds)
set.seed(1234)
iC<-training.import
iC$d2H.m3<-predict(bst, dtrain, nrounds = nrounds)
