...
predictors<-c((as.character(xgb.vars.2$x)),"d18O.m2","d2H.m2")
...
dxs.pp.3 <- caret::preProcess(subset(training,select=-c(dxs)), # last column is the outcome 
                            method = c("center", "scale"))
dxs.pp.3 #this will show which columns were centered and scaled
saveRDS(dxs.pp.3,file="dxs/dxs.pp.3")


training.tr <- predict(dxs.pp.3, newdata = subset(training,select=-c(dxs)))
training.target<-subset(training,select=c(dxs))

test.tr <- predict(dxs.pp.3, newdata = subset(test,select=-c(dxs)))
test.target<-subset(test,select=c(dxs))

watch.tr <- predict(dxs.pp.3, newdata = subset(watch,select=-c(dxs)))
...
write.csv(xgb_grid30, file = "dxs/dxs.xgb_grid30.3_srv.csv",row.names=F)
...
saveRDS(bst,file="dxs/dxs.model3RDS")
dxs.featureIDs<-as.character(bst$feature_names)
saveRDS(dxs.featureIDs,file="dxs/dxs.featureIDs.3")
xgb.save(bst,"dxs/dxs.model3XGB")
...
write.csv(importance, file = paste("dxs/xgb.importance.3",tuneround,".csv"))

write.csv(xgbDart.tuning30, file = "dxs/dxs.xgbDart.tuning30.3.csv")  
...
set.seed(1234)
iA<-test.import
iA$dxs.m3<-predict(bst, dtest, nrounds = nrounds)
set.seed(1234)
iB<-watch.import
iB$dxs.m3<-predict(bst, dwatch, nrounds = nrounds)
set.seed(1234)
iC<-training.import
iC$dxs.m3<-predict(bst, dtrain, nrounds = nrounds)
...
i<-rbind(iA,iB,iC)
i<-i[names(i)%in%c("Site","Date","dxs.m3","model.group")]
...
write.csv(GNIP, file="dxs/GNIP.dxs.3.csv")
