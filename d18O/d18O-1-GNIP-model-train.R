...
d18O.pp <- caret::preProcess(subset(training,select=-c(d18Of)), # last column is the outcome 
                            method = c("center", "scale"))
d18O.pp #this will show which columns were centered and scaled
saveRDS(d18O.pp,file="d18O/d18O.pp")


training.tr <- predict(d18O.pp, newdata = subset(training,select=-c(d18Of)))
training.target<-subset(training,select=c(d18Of))

test.tr <- predict(d18O.pp, newdata = subset(test,select=-c(d18Of)))
test.target<-subset(test,select=c(d18Of))

watch.tr <- predict(d18O.pp, newdata = subset(watch,select=-c(d18Of)))
...
write.csv(xgb_grid30, file = "d18O/d18O.xgb_grid30_srv.csv")
...
saveRDS(bst,file="d18O/d18O.model1RDS")
d18O.featureIDs<-as.character(bst$feature_names)
saveRDS(d18O.featureIDs,file="d18O/d18O.featureIDs")
xgb.save(bst,"d18O/d18O.model1XGB")
...
write.csv(xgbDart.tuning30, file = "d18O/d18O.xgbDart.tuning30.csv")  
...
set.seed(1234)
iA<-test.import
iA$d18O.m<-predict(bst, dtest, nrounds = nrounds)
set.seed(1234)
iB<-watch.import
iB$d18O.m<-predict(bst, dwatch, nrounds = nrounds)
set.seed(1234)
iC<-training.import
iC$d18O.m<-predict(bst, dtrain, nrounds = nrounds)
...
i<-rbind(iA,iB,iC)
i<-i[names(i)%in%c("Site","Date","d18O.m","model.group")]
...
write.csv(GNIP, file="d18O/GNIP.d18O.csv")
