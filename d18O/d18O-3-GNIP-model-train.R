...
training.import<-read.csv("training.3.csv",stringsAsFactors = F,row.names = 1)
training.import$Date<-as.Date(training.import$Date)
test.import<-read.csv("test.3.csv",stringsAsFactors = F,row.names = 1)
test.import$Date<-as.Date(test.import$Date)
watch.import<-read.csv("watch.3.csv",stringsAsFactors = F,row.names = 1)
...
predictors<-c((as.character(xgb.vars.2$x)),"dxs.m2")
...
d18O.pp.3 <- caret::preProcess(subset(training,select=-c(d18Of)), # last column is the outcome 
                            method = c("center", "scale"))
d18O.pp.3 #this will show which columns were centered and scaled
saveRDS(d18O.pp.3,file="d18O/d18O.pp.3")


training.tr <- predict(d18O.pp.3, newdata = subset(training,select=-c(d18Of)))
training.target<-subset(training,select=c(d18Of))

test.tr <- predict(d18O.pp.3, newdata = subset(test,select=-c(d18Of)))
test.target<-subset(test,select=c(d18Of))

watch.tr <- predict(d18O.pp.3, newdata = subset(watch,select=-c(d18Of)))
...
write.csv(xgb_grid30, file = "d18O/d18O.xgb_grid30.3_srv.csv",row.names=F)
...
saveRDS(bst,file="d18O/d18O.model3RDS")
d18O.featureIDs<-as.character(bst$feature_names)
saveRDS(d18O.featureIDs,file="d18O/d18O.featureIDs.3")
xgb.save(bst,"d18O/d18O.model3XGB")
...
write.csv(importance, file = paste("d18O/xgb.importance.3",tuneround,".csv"))

write.csv(xgbDart.tuning30, file = "d18O/d18O.xgbDart.tuning30.3.csv")  
...
set.seed(1234)
iA<-test.import
iA$d18O.m3<-predict(bst, dtest, nrounds = nrounds)
set.seed(1234)
iB<-watch.import
iB$d18O.m3<-predict(bst, dwatch, nrounds = nrounds)
set.seed(1234)
iC<-training.import
iC$d18O.m3<-predict(bst, dtrain, nrounds = nrounds)
...
i<-rbind(iA,iB,iC)
i<-i[names(i)%in%c("Site","Date","d18O.m3","model.group")]
