...
d18O.pp.2 <- caret::preProcess(subset(training,select=-c(d18Of)), # last column is the outcome 
                            method = c("center", "scale"))
d18O.pp.2 #this will show which columns were centered and scaled
saveRDS(d18O.pp.2,file="d18O/d18O.pp.2")


training.tr <- predict(d18O.pp.2, newdata = subset(training,select=-c(d18Of)))
training.target<-subset(training,select=c(d18Of))

test.tr <- predict(d18O.pp.2, newdata = subset(test,select=-c(d18Of)))
test.target<-subset(test,select=c(d18Of))

watch.tr <- predict(d18O.pp.2, newdata = subset(watch,select=-c(d18Of)))
...
write.csv(xgb_grid30, file = "d18O/d18O.xgb_grid30.2_srv.csv",row.names=F)
...
saveRDS(bst,file="d18O/d18O.model2RDS")
d18O.featureIDs<-as.character(bst$feature_names)
saveRDS(d18O.featureIDs,file="d18O/d18O.featureIDs.2")
xgb.save(bst,"d18O/d18O.model2XGB")
...
write.csv(importance, file = paste("d18O/xgb.importance.2",tuneround,".csv"))

write.csv(xgbDart.tuning30, file = "d18O/d18O.xgbDart.tuning30.2.csv")  
...
set.seed(1234)
iA<-test.import
iA$d18O.m2<-predict(bst, dtest, nrounds = nrounds)
set.seed(1234)
iB<-watch.import
iB$d18O.m2<-predict(bst, dwatch, nrounds = nrounds)
set.seed(1234)
iC<-training.import
iC$d18O.m2<-predict(bst, dtrain, nrounds = nrounds)
...
i<-rbind(iA,iB,iC)
i<-i[names(i)%in%c("Site","Date","d18O.m2","model.group")]
...
write.csv(GNIP, file="d18O/GNIP.d18O.2.csv")
